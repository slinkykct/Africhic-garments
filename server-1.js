...
app.patch('/api/admin/orders/:id/status', admin, async (req, res) => {
  try {
    ...
    try {
      await client.query('BEGIN');
      // Auto-generate tracking number BEFORE building query
      let finalTrackingNum = trackingNumber;
      if (status === 'shipped' && !finalTrackingNum) {
        finalTrackingNum = genTrackingNum();
      }
      // Build SET clause — id is ALWAYS $2, other params start at $3+
      const setClauses = ['status=$1', 'updated_at=NOW()'];
      const params = [status, req.params.id];
      if (finalTrackingNum) { params.push(finalTrackingNum); setClauses.push(`tracking_number=$${params.length}`); }
